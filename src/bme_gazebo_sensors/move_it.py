...
class WhitePointImageVisualizer(Node):
    ...
    def detect_horizontal_line(self, image, angle_tolerance_deg=10, min_length=100):
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        edges = cv2.Canny(gray, 50, 150, apertureSize=3)

        lines = cv2.HoughLinesP(edges, 1, np.pi / 180, threshold=50, minLineLength=min_length, maxLineGap=20)
        if lines is not None:
            for line in lines:
                x1, y1, x2, y2 = line[0]
                angle = math.degrees(math.atan2(y2 - y1, x2 - x1))
                if abs(angle) < angle_tolerance_deg:  
                    self.get_logger().info(f"Detected horizontal-ish line at angle {angle:.2f}°")
                    return True
        return False
    ...
    def pointcloud_callback(self, msg):
        ...
        if len(centers) == 2:
            ...
            if self.detect_horizontal_line(white_img):
                if white_ground_points:
                    distances = [math.sqrt(p[0]**2 + p[1]**2) for p in white_ground_points if p[0] > 0]
                    
                    if distances:
                        min_distance = min(distances)
                        self.get_logger().info(f"Distance to horizontal white line: {min_distance:.2f} m")

                        if min_distance <= 0.3:
                            self.get_logger().warn("Within 30 cm of stop line — stopping robot.")
                            cmd.linear.x = 0.0
                            cmd.angular.z = 0.0
                            self.publish(cmd)
                            return
                            ...
                            # Smooth deceleration zone: 60 cm to 30 cm
                            max_speed = 0.3  # original forward speed
                            min_speed = 0.05  # very slow approach speed
                            # Linear deceleration
                            speed = min_speed + (max_speed - min_speed) * ((min_distance - 0.3) / 0.3)
                            cmd.linear.x = max(min_speed, min(max_speed, speed))
                            cmd.angular.z = 0.0
                            self.publish(cmd)
                            self.get_logger().info(f"Decelerating: distance = {min_distance:.2f} m, speed = {cmd.linear.x:.2f} m/s")
                        else:
                            # Far from stop line — move normally
                            cmd.linear.x = 0.3
                            cmd.angular.z = 0.0
                            self.publish(cmd)
                    else:
                        self.get_logger().info("No white points in front to compute distance.")
            else:
                self.publish(cmd)
