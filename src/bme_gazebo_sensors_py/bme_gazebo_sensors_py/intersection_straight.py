...
INITIAL_INTERSECTION_FORWARD_MOVEMENT           = 5                  # metres
LEFT_TURN_ANGULAR_SPEED                         = 0.1                # rad/s (+ve = CCW = left)
TURN_ANGLE                                      = radians(0)         # 90 was over-turning for me? I'm not sure why though
# ──────────────────────────────────────────────────────────────────────────────
...
class IntersectionStraightDriver(Node):
	def __init__(self):
		...
		self.should_drive = True ####### CHNAGE LATER #############################
		...
		if DEBUG:
			self.get_logger().info(f"⏩ Driving {INITIAL_INTERSECTION_FORWARD_MOVEMENT:g} m.")

	def intersection_cb(self, msg: String):
		if msg.data.lower() == "straight":
			self.should_drive = True
			self.get_logger().info("🛑 Received 'straight' from /intersection. Activating straight-turn behavior.")
		else:
	...
	def pointcloud_cb(self, msg: PointCloud2):
		...
		if DEBUG:
			if len(pts_xy) > 0:
				...
				BLUE_RGB_FLOAT = struct.unpack('f', struct.pack('I', BLUE_RGB_INT))[0]
		...
		wanted =  normalise_angle(self.prev_yaw - self.turn_start_yaw)
		...
		if self.raw_turn_completed and  (dists == -1).all():
			self.get_logger().info("No rays inside ANGLE_TOLERANCE band")           #################### NEED TO RESET VELOCITY OR DO SOMETHING HERE
																					###################### BOT CONTINUES TO GO WHEREVER IT USED TO GO
			return

		never_hit = (~hit_mask) & on_lane_dir

		# Define your small‐angle band and distance threshold:
		CHECKING_SMALL_ANGLE   = np.deg2rad(1.0)
		DISTANCE_THRESHOLD_PX = 5  # <-- set this to whatever threshold you like

		# Compute which rays lie within ±1° of the wanted heading:
		angle_diffs = np.abs([normalise_angle(a - wanted) for a in angles])
		in_straight_band = angle_diffs <= CHECKING_SMALL_ANGLE

		# If there are any rays in that band, and *all* are above threshold, go straight:
		if in_straight_band.any() and np.all(dists[in_straight_band] > DISTANCE_THRESHOLD_PX):
			# Pick the ray in that band whose angle is *closest* to wanted:
			band_idxs = np.flatnonzero(in_straight_band)
			best_idx  = int(band_idxs[np.argmin(angle_diffs[band_idxs])])
